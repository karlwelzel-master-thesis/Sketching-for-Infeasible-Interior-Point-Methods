...
Note that for example, for \(w = 2m\) about \(25\%\) to \(40\%\) of the entries of the sketched matrix are nonzero which is not very sparse compared to the density of \(\mat{A}\) at around \(0.2\%\).
...
It turns out, that randomly drawing the row indices for all columns without replacement is much faster than drawing them with replacement for each column separately, so it was beneficial to generate slightly more row indices than needed and then discard those instance where collisions occured.
...
Because the available sparse QR implementation did not bring any performance gains a QR decomposition algorithm for dense matrices was used whose runtime, unsurprinsingly, mainly depends on the matrix dimension, i.e. on \(w\).
