...
Next, a random feasible point \((\vek{x}, \vek{y}, \vek{s})\) was generated using standard normal and uniform distributions for the entries and \(\vek{b}\) and \(\vek{c}\) were set accordingly to ensure feasibility and boundedness of \cref{eqn:primal-lp-instance}:
\begin{equation}
  \vek{b} = \mat{A} \vek{x} \quad \text{and} \quad \vek{c} = \mat{A}^T \vek{y} + \vek{s}
\end{equation}
