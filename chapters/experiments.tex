...
% TODO: Scrap all these options, rename to "final comparison" and focus on table
...
To finish this chapter, a direct comparison between our iterative approach and the direct solvers is due.
...
\Cref{table:runtime_comparison} shows that while computing the QR of the sketched matrix \(\mat{W}\mat{D}\mat{A}^T\) is much faster than computing the QR decomposition of the full matrix \(\mat{D}\mat{A}^T\) the preconditioned CG method cannot compete with the Cholesky decomposition of \(\mat{A}\mat{D}\mat{A}^T\).
