...
To simplify the runtime bounds and the following discussion, we will from now on assume that \(\e < 1/2\) is fixed and furthermore assume that \(\mat{W} \in \R^{w \times n}\) is a sparse embedding matrix and a \((1\pm\e)\) \(\ell_2\)-subspace embedding of the column space of \(\mat{B} \in \R^{n \times m}\).
...
\begin{algorithm}
  ...
  Construct a random sparse embedding matrix \(\mat{W} \in \R^{w \times m}\) as in \cref{thm:sparse-ose} for the given \(\delta\) and \(\e\)\;\label{line:draw-sketching-matrix}
